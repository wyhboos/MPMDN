...
ompl::base::PlannerStatus ompl::geometric::MPPN::solve(const base::PlannerTerminationCondition &ptc)
{
    ...
    auto path = bidirectional_plan(start.get(), goal.get());
    int l = path.size();
...
std::vector<ompl::base::ScopedState<>*> ompl::geometric::MPPN::replan_with_nn(std::vector<ompl::base::ScopedState<>*> path_ori)
{
    ...
    for (int i = 0; i < (l-1); i++)
    {
        if(!si_->isValid(path_ori[i])) continue;

        if(si_->checkMotion(path_ori[i], path_ori[i+1]))
        {
            path_new.pushback(path_ori[i]);
            path_new.pushback(path_ori[i+1]);
        }
        else
        {
